...
function User() {  
  ...
  const handleDelete = async (id) => {
    // alert(id);
    const userConfirmed = confirm("Do you want to delete this inspector?");
    if (userConfirmed) {
      // Perform the action when the user clicks OK
      const formData={id};
      const response = await vehicleApi.deleteInspector(formData);
      if (response.status === 200 && response.data.status === 200 && response.data.success === true) {
        // console.log(response);
              ...
              <Box className={dashboardStyles.tm_auctionvehicle_table_main}>
                <Box className={dashboardStyles.tm_auctionvehicle_table_main_top}>
                  <Box className={dashboardStyles.tm_auctionvehicle_table_main_top_title}>
                    <Typography variant='h4'> Inspector List</Typography>
                  </Box>
                  <Box className={dashboardStyles.tm_auctionvehicle_table_main_top_btn}>
                  <Link href="/dashboard/inspectors/create"><Button variant="contained">Add</Button></Link>
                  </Box>
                </Box>
                ...
                <Paper sx={{ width: '100%', overflow: 'hidden' }}>            
                  <TableContainer sx={{ maxHeight: 700 }}>
                    <Table stickyHeader aria-label="sticky table">
                      <TableHead>
                        <TableRow>
                          <TableCell align="center" colSpan={8}>Inspector List</TableCell>
                        ...
                        <TableRow>
                            ...
                            <TableCell  align="center" style={{ top: 57, minWidth: 170 }}>Email</TableCell>
                      ...
                      <TableBody>
                                {userList.map((data,key) => (
                                    <TableRow key={key}>
                                      ...
                                      {data.createdAt ? (<>
                                        <TableCell align="center">{format(new Date(data.createdAt), 'dd/MM/yyyy')}</TableCell>
                                      </>):(<>
